...
module.exports = {
  create(req, res) {
    ...
    fixoService.create(dados, (err, result) => {
      ...
      res.status(201).json({ message: 'Gasto fixo criado com sucesso', id: result.insertId });
    });
  },
  ...
  findById(req, res) {
    fixoService.findById(req.params.id, (err, row) => {
      if (err) return res.status(500).json({ error: err.message });
      if (!row) return res.status(404).json({ message: 'Gasto fixo não encontrado' });
  ...
  update(req, res) {
    fixoService.update(req.params.id, req.body, (err, result) => {
      if (err) return res.status(500).json({ error: err.message });
  ...
  delete(req, res) {
    fixoService.delete(req.params.id, (err, result) => {
      if (err) return res.status(500).json({ error: err.message });

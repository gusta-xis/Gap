...
module.exports = {
  create(req, res) {
    salarioService.create(req.body, (err, result) => {
      ...
      if (req.passo)
        req.passo(
          '💾',
          `Salvo no Banco (ID: ${result.insertId || 'Múltiplos'})`
        );
  ...
  findById(req, res) {
    salarioService.findById(req.params.id, (err, row) => {
      if (err) return sendError(res, err);
      if (!row)
        return res.status(404).json({ message: 'Salario não encontrado' });
  ...
  update(req, res) {
    salarioService.update(req.params.id, req.body, (err) => {
      if (err) return sendError(res, err);
  ...
  delete(req, res) {
    salarioService.delete(req.params.id, (err) => {
      if (err) return sendError(res, err);
      return res.status(200).json({ message: 'Salario deletado com sucesso' });
    });
  },
};
